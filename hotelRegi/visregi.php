...
if( $conn ) {
    ...
    $sql= "Select top 1 CustomerID FROM NONMEMBER order by CustomerID desc";
    $id = "";
   ...
   $id = smartcounter($id);
    ...
    $params= array(array($id,SQLSRV_PARAM_IN), array($_POST['vis_ln'],SQLSRV_PARAM_IN), array($_POST['vis_mn'],SQLSRV_PARAM_IN), array($_POST['vis_fn'],SQLSRV_PARAM_IN),  array($_POST['vis_contact'],SQLSRV_PARAM_IN), array($_POST['vis_email'],SQLSRV_PARAM_IN));

    $stmt = sqlsrv_query($conn,'{CALL SP_NonMember(?, ?, ?, ?, ?, ?)}', $params);
