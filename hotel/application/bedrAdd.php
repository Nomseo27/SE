...
if( $conn ) {
   ...
   $id = smartcounter($id);
    ...
    $params= array(array($id,SQLSRV_PARAM_IN), array('BEDROOM',SQLSRV_PARAM_IN), array($_POST['roomno'],SQLSRV_PARAM_IN), array($_POST['Price'],SQLSRV_PARAM_IN),  array($_POST['rate'],SQLSRV_PARAM_IN), array($_POST['roomdes'],SQLSRV_PARAM_IN));

    $stmt = sqlsrv_query($conn,'{CALL SP_BEDROOM(?, ?, ?, ?, ?, ?)}', $params);
